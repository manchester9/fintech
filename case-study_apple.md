## What is the background of your company? 
* Apple Pay is the leading mobile payment and digital service

## What do they do? 
* Mobile payment 

## Who are their competitors?
* Google Pay

## How does this company use technology to their advantage? 
* Company uses a combination of finance and mobile payment systems to deliver value

## Why are they considered a FinTech company?
* Combination of finance and technology

## Why is this company exemplary in the FinTech space? 
* It is the leader in mobile payment and digital services

## What are their strengths and weaknesses?
* Strenghts is providing an easy way to pay for things
* Weakness - 

## What is the prognosis for this company? 
* Leader in space

## Where do they expect to be in the future?
* Implemented as a normal way to do business

#  Apple Pay: Leader in mobile payments

### Apply Pay logo 
* ![ApplePay](data:image/png;base64,)

 ## Description of company 
 * Apple Pay is arguably the leading mobile payment and digital wallet service. It allows users to make payments in person, in iOS apps, and on the web. It is supported on the iPhone, Apple Watch, iPad, and Mac

## Description of content 
* Content for the description

## Contributor
* Roland

### Apply Pay: Leader in mobile payments
* More secure than a traditional credit card
* Possible to store additional information that can be needed for certain transactions, such as drivers license information to verify age when purchasing alcoholic beverages